# Source: Google
population = {
    "Algeria": 41.32 * 1000000, 
    "Australia": 24.6 * 1000000,
    "Austria": 8.822 * 1000000,

    "Bahrain": 1.493 * 1000000,
    'Belarus': 9.485 * 1000000,
    "Belgium": 11.3 * 1000000,
    "Brazil": 209.3 * 1000000,

    "Canada": 37.59 * 1000000,
    "Chile": 18.05 * 1000000,
    "China": 1.386 * 1000000000,
    "Czechia": 10.65 * 1000000,

    "Denmark": 5.603 * 1000000,
    "Dominican Republic": 10.77 * 1000000,

    "Ecuador": 16.62 * 1000000,
    "Egypt": 97.55 * 1000000,

    "France": 66.99 * 1000000,

    "Germany": 82.79 * 1000000,

    "India": 1.339 * 1000000000,
    "Indonesia": 264 * 1000000,
    "Iran": 81.16 * 1000000,
    "Iraq": 38.27 * 1000000,
    "Ireland": 4.83 * 1000000,
    "Israel": 8.712 * 1000000,
    "Italy": 60.48 * 1000000,

    "Japan": 126.8 * 1000000,

    "Korea, South": 51.47 * 1000000,

    "Malaysia": 31.62 * 1000000,
    "Mexico": 129.2 * 1000000,

    "Netherlands": 17.18 * 1000000,
    "Norway": 5.368 * 1000000, 

    'Pakistan': 212.2 * 1000000,
    "Peru": 32.17 * 1000000,
    "Philippines": 104.9 * 1000000,
    "Poland": 37.98 * 1000000,
    "Portugal": 10.29 * 1000000,

    'Qatar': 2.782 * 1000000,

    "Romania": 19.53 * 1000000,
    "Russia": 144.5 * 1000000,

    "San Marino": 33400,
    "Saudi Arabia": 33.7 * 1000000,
    "Singapore": 5.612 * 1000000,
    "Spain": 46.66 * 1000000,
    "Sweden": 10.12 * 1000000,
    "Switzerland": 8.57 * 1000000,

    "Taiwan*": 23.78 * 1000000,
    "Thailand": 69.04 * 1000000, 
    "Turkey": 80.81 * 1000000,

    "United Arab Emirates": 9.4 * 1000000,
    "United Kingdom": 66.44 * 1000000,
    "US": 327.2 * 1000000,
}
